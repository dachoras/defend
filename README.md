...
# <img src="assets/icon.png" width="32" height="32" valign="middle"> Defend

Interactive self-hosted web security playground and defense simulator.
...
docker run -d \
  ...
  -e DEFEND_PIN=your_secret_pin \
  ghcr.io/studio2201/defend:latest
```
...
| :--- | :--- | :--- |
| `PORT` | The network port the web server binds to | `4504` |
| `DEFEND_PIN` | Security PIN code required for client authentication | (None) |
| `DEFEND_DATA_DIR` | Directory path where persistent data is stored | `/config` |
...
Each container includes a built-in admin tool located in the system path as `defend`. To open the console, execute a shell inside the container:
```bash
docker exec -it <container-name> sh
```
Then, run `defend` to manage the application:
```bash
defend [command]
```
Running `defend` without arguments or running `defend tui` launches the interactive terminal user interface.
