...
|---|---|---|
| `PORT` | The port number the backend HTTP server will bind to. | `4504` |
| `DEFEND_PIN` | Optional PIN to restrict access to the application interface. | (None) |
| `DEFEND_DATA_DIR` / `DATA_DIR` | Path to the directory where application data and database files are stored. | `data` / `/config` |
| `DEFEND_ALLOWED_ORIGINS` / `ALLOWED_ORIGINS` | Comma-separated list of allowed CORS origins, or `*` to allow all. | `*` |
...
```bash
docker run -d -p 4504:4504 -v /path/to/appdata:/app/data ghcr.io/studio2201/defend:latest
```
