...
async fn metrics_endpoint_returns_prometheus_text_with_expected_lines() {
    ...
    assert!(text.contains("defend_requests_total "));
    assert!(text.contains("defend_requests_429_total "));
    assert!(text.contains("defend_active_sessions 2\n"));
    assert!(text.contains("defend_leaderboard_entries 1\n"));
    assert!(text.contains("defend_build_info{version=\"test\"} 1\n"));
}
