...
mod tests {
    ...
    async fn metrics_endpoint_renders_prometheus_text() {
        ...
        let mut server = crate::config::AppConfig::load_from_env(4401);
        server.base_url = "http://localhost:4501".to_string();
        let mut cfg = AppConfig::load_from_env(4401);
        ...
        assert!(
            text.contains("defend_requests_total 3\n"),
            "missing counter; body was: {text}"
        );
        assert!(text.contains("defend_requests_429_total 0\n"));
        assert!(text.contains("defend_leaderboard_entries 1\n"));
        assert!(text.contains("defend_build_info{version=\"9.9.9\"} 1\n"));
    }
}
