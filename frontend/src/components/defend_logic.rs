...
impl GameState {
    ...
    pub fn update(&mut self) {
        ...
        if self.powerup_type > 0 {
            ...
            if self.powerup_y > 100.0 { self.powerup_type = 0; }
                ...
                if self.powerup_type == 1 {
                    self.player_shield = (self.player_shield + 20).min(100);
                    self.planet_shield = (self.planet_shield + 20).min(100);
        ...
        if let Some(mut bh) = self.boss_health {
            ...
            if self.ticks % 40 == 0 { self.threats.push(Threat { x: self.boss_x, y: 17.0, speed: 1.2, size: 1.2 }); }
        ...
        if self.boss_health.is_none() {
            ...
            if self.ticks.is_multiple_of(spawn_interval) {
                ...
                self.threats.push(Threat { x, y: 0.0, speed: js_sys::Math::random() * 0.4 + 0.3 + (self.wave as f64 * 0.05), size: js_sys::Math::random() * 2.0 + 2.0 });
            }
        }
        ...
        for threat in old_threats {
            if threat.y >= 90.0 && threat.y <= 95.0 && (threat.x - self.player_x).abs() < 5.0 {
                self.player_shield = self.player_shield.saturating_sub(20);
