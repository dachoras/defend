...
//! We re-export it here so that consumers in the Defend crate can use the
//! same `use crate::components::footer::Footer` path pattern as for every
//! other component, and so that any future Defend-specific footer
...
//! flag (`bool`), and an HTML `children` slot. The Defend UI currently
...
//! this module rather than to the shared crate, so the Defend variant
