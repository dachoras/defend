# =============================================================================
# defend — Alpine container image (Unraid-compatible)
# =============================================================================
...
#     DEFEND_DATA_DIR env var if needed.
#   - Listens on 0.0.0.0:<port> for bridge networking.
#
# Build (Podman):
#   cd ~/projects/studio2201/defend
#   podman build -f Containerfile.ubi -t defend:latest .
#
# Run (Unraid-style):
#   podman run -d --name defend \
#     -p <host-port>:4504 \
#     -v /mnt/user/appdata/defend:/config \
#     -e DEFEND_PIN=<your-pin> \
#     defend:latest
#

# Compare size:
#   podman images | grep -E 'defend|studio2201/defend'
# =============================================================================
...
LABEL org.opencontainers.image.title="defend" \
      org.opencontainers.image.description="defend (Alpine, built on Alpine; Unraid-compatible)" \
      org.opencontainers.image.source="https://github.com/studio2201/defend" \
...
COPY --from=builder /src/target/release/sh /usr/local/bin/defend
RUN chmod +x /usr/local/bin/defend
...
ENV PORT=4504 \
    RUST_LOG=info \
    DEFEND_DATA_DIR=/config


EXPOSE 4504

HEALTHCHECK --interval=30s --timeout=5s --start-period=10s --retries=3 \
  CMD wget -qO- "http://127.0.0.1:${PORT:-4504}/health" >/dev/null || exit 1
